...
;      redef_prof
; Purpose
;      Re-define the radius array, to meet the ionic fraction
;      calculation accuracy due to the stability of the electron
;      temperature, and plot the velocity and temperature profiles
;      of the adiabatic expansion motion, and compare it to the
;      exponential approximation.
;
; Written by sw, Jun 13, 2019
;-

  ;System parameter
  rootpath='~/data/emcal/model/cc85/analytical/adiabatic/'+ $
    'adiabatic_nei/adiabatic/nei_case/accu_check/redef_model/'
  ...
  ; T =reverse(dindgen(500)+1)/500.*T0
  T =10^(-findgen(2501)/1000.)*T0 ;in K
  ...
  xrange=[0.95,70]*r0 & yrange=[0.85,1.1] & resrange=[-1,1]*0.12
  ...
  plot,Tr,Tr_u-Tr,xrange=xrange,yrange=[4e-4,4e-2],xstyle=1,ystyle=1, $
  ...
  plot,Tr,tau_app/1e7,xrange=xrange,yrange=[0,1.2],xstyle=1,ystyle=1, $
  ...
  plot,Tr,accum_tau_app/1e8,xrange=xrange,yrange=[0.1,1e2],xstyle=1,ystyle=1, $
