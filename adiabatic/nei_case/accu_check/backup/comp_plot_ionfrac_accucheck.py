...
for Z in Zlist:
  ...
  for i in ion_range:
    if i == 5:
      ...
      linestyle='dashed'
    plt.plot(radius1, np.abs(nei1_ionfrac[Z][i,0:ncondi1] - \
      nei2_ionfrac[Z][i,0:ncondi2]), color='C'+"%s"%(i-begin_ion), \
      linewidth=2, label=elsymb+' '+pyatomdb.atomic.int2roman(i+1), \
      linestyle=linestyle)
  plt.xlabel('Radius (pc)')
  plt.ylabel('Ionic Fraction')
  ...
  plt.yscale('log')
  plt.legend(loc=0)
  plt.savefig(rootpath+'figures/comp_iondiff_comp_%s.png' %elsymb)
  plt.close()
